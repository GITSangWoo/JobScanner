...
# File for data input after preprocessing in service DB notice table 
...
def insert_notice_data():
    ...
    try:
        ...
        # 1. combined_table과 extract_tech_stack을 조인하는 쿼리 작성
        # tot_tech가 빈 JSON이 아닌 경우만 포함하도록 조건 추가
        query = """
        SELECT 
        ...
        # 2. 쿼리 실행하여 데이터를 가져옵니다.
        cursor.execute(query)
        rows = cursor.fetchall()

        # 3. 디버깅: 가져온 첫 번째 행 출력
        print("First row:", rows[0] if rows else "No rows fetched.")
        ...
        for row in rows:
            ...
            if len(row) >= 10:  # 최소 10개의 컬럼이 있어야 한다고 가정 (id 포함)
